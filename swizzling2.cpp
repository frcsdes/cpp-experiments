...
constexpr std::size_t axis_index(char axis) {
    switch (axis) {
        case 'x': return 0;
        case 'y': return 1;
        case 'z': return 2;
...
struct Vec : std::array<T, N> {
    ...
    constexpr auto operator[](char const (&axes)[M]) const {
        return [this, &axes]<std::size_t... I>(std::index_sequence<I...>) {
            return Vec<M - 1, T>{this->at(axis_index(axes[I]))...};
    ...
    constexpr auto operator[](char const (&axes)[M]) {
        return [this, &axes]<std::size_t... I>(std::index_sequence<I...>) {
                return Ref{this->at(axis_index(axes[I]))...};
        }(std::make_index_sequence<M - 1>{});
    }
...
int main() {
    ...
    auto constexpr reverse = sequence["zyx"];
